...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Стоимость попытки: {spin_cost} монет.",
    ...
    if query.data == "spin":
        if coins < spin_cost:
            ...
            if is_jackpot:
                ...
                if loss_streak >= 5:
                    coins += 10
                    result += "\n🔥 Утешительный бонус за серию проигрышей: +10 монет!"
                    loss_streak = 0
            ...
            await query.answer()
            await query.edit_message_text(
                f"{result}\nВаш баланс: {coins} монет.\nСтоимость следующей попытки: {spin_cost} монет.",
                reply_markup=main_menu(spin_cost)
            )
...
# Запуск приложения
app = ApplicationBuilder().token("7967825975:AAFc3xIK8RUHMlfQpqRsNjc1AGHxlmj4aes").build()
